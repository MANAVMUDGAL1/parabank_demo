...
*** Test Cases ***
Validate Account balance Credited via Api Test
    ...
    ${before}=    Get Account Balance via API    ${CUSTOMER_ID}    ${To_acc_id}
    Transfer Funds via API   ${ACCOUNT_ID}    ${To_acc_id}    50
    Sleep    2s
    ${after}=    Get Account Balance via API    ${CUSTOMER_ID}    ${To_acc_id}
